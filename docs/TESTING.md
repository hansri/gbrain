...
|---|---|---|---|
| `bun run test` | Parallel unit-test fast loop. Up to 4 shard processes via `scripts/run-unit-parallel.sh`, with a shared default worker budget of `min(CPUs, 8)`, then a serial pass over `*.serial.test.ts`. Excludes `*.slow.test.ts` and `test/e2e/*`. No pre-checks, no typecheck. | Host-dependent; bounded to avoid PGLite/WASM memory contention | Inner edit loop. Default. |
...
If a shard wedges (per-shard `GBRAIN_TEST_SHARD_TIMEOUT` cap, default 1500s), the wrapper writes `--- shard N: WEDGED after ${SHARD_TIMEOUT}s ---` to the failure log, includes the last 50 lines of the shard log, and proceeds with other shards' results. This classification also works on macOS without GNU `timeout`: timeout provenance is recorded before TERM, so a child that traps TERM and exits zero still fails as `rc=124`. By default, the wrapper divides a total worker budget of `min(detected CPUs, 8)` across its shard processes, reduces the shard count when necessary to honor that ceiling, and caps each shard at two workers. `--max-concurrency` or `GBRAIN_TEST_MAX_CONCURRENCY` is an explicit operator override and the startup line always reports both the effective total and budget.

### File taxonomy

- `*.test.ts` → fast loop (up to 4 shard processes with bounded internal concurrency).
...
- `test/files-object-safety.test.ts` — PGLite regressions for storage-required metadata, immutable content-addressed replacements, DB-failure preservation of the prior pointer/object, verified+fsynced atomic restore (including corrupt-download and failed-promotion recovery), and `files verify` downloading every referenced object for exact byte-count + SHA-256 checks.
- `test/import-file.test.ts` — import pipeline.
- `test/upgrade.test.ts` — schema migrations.
- `test/file-migration.test.ts` — file migration.
- `test/file-resolver.test.ts` + `test/files-object-safety.test.ts` — file resolution and migration, including strict redirect metadata/content verification, valid-pointer precedence over a partial local file, durability ordering, and a writer racing after the final revision comparison (the newer local file survives and the stale pointer is retired).
...
- `test/sync-lookup-failure.serial.test.ts` — source-ownership lookup failure and bookmark-preservation regression. SERIAL because repeated deliberate resolver failures leave PGLite/WASM state unsuitable for a later import in the same process; a fresh process preserves the production assertion without poisoning unrelated tests.
- `test/sync-rename-rollback.serial.test.ts` — final-write rename rollback and retry regression. SERIAL because this fault-injection seam mutates the PGLite engine method table and has demonstrated an order-dependent WASM hang when run after the larger sync lifecycle suite; its own fresh process is stable.
...
- `test/oauth.test.ts` — OAuth 2.1 provider: register, getClient, `client_credentials` grant exchange, `authorization_code` flow with PKCE challenge/verifier, refresh token rotation, `verifyAccessToken` with both OAuth + legacy `access_tokens` fallback, including legacy capability scopes/exact tools and malformed-field denial, `revokeToken`, `sweepExpiredTokens`; contract test asserting `scope` + `localOnly` annotations on all operations; `coerceTimestamp` unit cases (null/undefined/string/number/throw-on-NaN); NULL-`expires_at`-as-expired contract for both refresh + access token paths; cascade-delete contract asserting `revoke-client` purges `oauth_tokens` + `oauth_codes` via FK CASCADE; cross-client isolation (wrong-client attempt MUST reject AND rightful owner MUST still succeed atomically afterward); empty-string `redirect_uri` bypass guard; PKCE DCR public-client gate (`token_endpoint_auth_method: "none"` returns no `client_secret`, default `client_secret_post` clients get the one-time-reveal secret, `getClient` NULL→undefined normalization, full PKCE `/authorize` → `/token` round-trip against a public client).
- `test/auth-create-args.test.ts` — strict legacy-token capability CLI parsing: create/permissions flag order, deduplication, missing and trailing arguments, unknown flags/actions/scopes, invalid tool names, and empty explicit lists all fail before a database write.
...
- `test/http-transport.test.ts` + `test/serve-http-legacy-capabilities.test.ts` — HTTP transports: bearer auth + missing/no-Bearer/unknown/revoked + `/health` bypass; exact legacy-token tool filtering; discovery intersects allowed tools with callable scopes; call scope rejection before dispatch; malformed explicit allow-lists deny all; dispatch.ts round-trip; invalid_params; application/json response shape (not SSE); CORS default-deny + allowlist; body cap on Content-Length AND chunked; two-bucket rate limit (refill, exhaust+Retry-After, LRU eviction, TTL prune, pre-auth IP fires before DB); `mcp_request_log` audit on success + auth_failed.
...
- `test/e2e/files-object-safety-postgres.test.ts` — real-Postgres file pointer replacement and rollback safety: immutable revision upload precedes the single DB pointer switch, metadata remains real JSONB, concurrent first writes serialize to one logical row, and a trigger-forced transaction failure preserves the prior referenced object while leaving only a safe unreferenced new revision.
...
- `test/e2e/pgbouncer-teardown.test.ts` — PgBouncer TRANSACTION-mode teardown (#2084 / the #1972→#2015→#2084 class). Pins the bug CLASS, not timings: a CLI op against a txn-mode pooled URL exits 0 with intact stdout and does NOT ride the 10s hard-deadline backstop (the `engine.disconnect() did not return` banner is the smoking gun — pre-#2084 it printed on 100% of query-shaped ops). Gated by `GBRAIN_TEST_DB=1` plus `GBRAIN_PGBOUNCER_URL` + `GBRAIN_PGBOUNCER_DIRECT_URL` (NOT `DATABASE_URL`) — set automatically by `bun run ci:local`'s `pgbouncer` compose service; skips gracefully elsewhere. The configured URLs must select `gbrain_test`; after validation, the test derives its DEDICATED `gbrain_pgbouncer` database so it never races the `gbrain_test` TRUNCATE fixtures.
...
- `test/e2e/serve-http-oauth.test.ts` — real-Postgres E2E against `gbrain serve --http`. Spawns a subprocess server, registers trusted clients through the local CLI, mints `client_credentials` tokens, and exercises the `/mcp` JSON-RPC pipeline. It proves network `/register` is unavailable, no-consent `/authorize` returns no redirect/code, discovery advertises only reachable grants, public token errors are opaque, and generated admin credentials never enter public or non-interactive logs. A real legacy bearer token with two exact tools proves discovery/call allowlisting; real CLI revocation proves tokens stop working; remote `shell` and `subagent` jobs remain blocked. Every subprocess receives an explicit environment because Bun's `execSync` does not inherit in-process `process.env` mutations. Cleanup is guarded so setup failures are not masked. Skips without `DATABASE_URL`.
- Current HTTP trust coverage additionally pins network `/register` unavailable, no-consent `/authorize` denied without redirect, reachable OAuth grant metadata, opaque token errors, generated-token log suppression, authenticated admin flows, source-header spoof rejection, server-derived queued source, and durable `remote`/untrusted evidence tags. The ingest handler tests assert the resulting page persists only under that source with source kind/URI/channel provenance and strips caller-forged trust markers.
